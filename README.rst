...
To do this, first clone this repository. Then run::
  ...
  docker build --build-arg VERSION_OCTOPUS=15.1 -t octimage .

to build Octopus version ``15.1`` in the container and create the Docker image with name ``octimage``.
...
instead of ``VERSION_OCTOPUS=15.1``. Omitting the ``VERSION_OCTOPUS`` argument
...
|stable| Debian Bookworm (12), Latest Octopus release (15.1) using autotools
